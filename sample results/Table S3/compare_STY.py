...
if __name__ == '__main__':
    ...
    file_name = "STY_sample.xlsx"
    sheet_name = "held-out subset"
    
    stds = read_excel_columns_to_json(file_name, sheet_name, ["Index", "Human Answer", "Human Basis"])
    results = read_excel_columns_to_json(file_name, sheet_name, ["Index", "DeepSeek Answer", "DeepSeek Basis"])
    ...
    for std in stds:
        for result in results:
            ...
            if std["Index"] == result["Index"]:
                ...
                if std["Human Basis"] == result["DeepSeek Basis"]:
                    s1 = compare_float_strings(std["Human Answer"], result["DeepSeek Answer"])
                    score_deepseek += s1
                    if s1 != 1:
